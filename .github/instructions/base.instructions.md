...
### Architectural Integrity (Binding Agreement)

> [!CAUTION]
> **This is a binding agreement, not a guideline.** It overrides the natural reflex to propose the smallest possible change.

For every non-trivial change, **the architecturally cleanest and most maintainable solution must be named explicitly first** — even if it is larger than a quick fix. The user then decides, with full information, whether to take the main path or — for concrete reasons (time pressure, scope) — an interim step. The choice belongs to the user, never to a convenience reflex.

**What this means in practice:**

1. **Symptom ≠ root cause.** When landing on the second hack in the same chain, **stop** and name the structural cause instead of proposing a third hack. Two hacks in the same area are the signal that the architecture, not the code, needs to change.
2. **Always make the target architecture explicit.** Even when proceeding "step by step", show the **whole map** — the user must be able to place each step in the larger picture. "Step by step" never means "hide the direction".
3. **Hacks need an expiration date.** When a pragmatic interim step is genuinely necessary, mark it as such (`TODO(<replacement>): <reason>` + planned end state) — never present it as the final solution.
4. **Respect third-party library boundaries.** Use the documented public API. The moment the work fights internals (undocumented CSS classes, private DOM structure, reflection over non-public members), that is the signal to **build a small own abstraction**, not to push harder.
5. **The user owns the trade-off decision.** The agent's job is to surface the choice with full context (effort, risk, maintenance cost of each option), not to pre-decide based on perceived effort.

> [!WARNING]
> Violating this agreement — silently taking the path of least resistance, hiding the architectural option, stacking hacks instead of naming the structural cause — is a defect. If the agent notices itself doing this mid-task, it must stop and surface the architectural option before continuing.
...
- For third-party UI libraries that don't natively support a layout/behavior requirement, build a small custom wrapper using the library's documented public API instead of escalating with `!important` / `::deep` against internals. See [UI / CSS Discipline](#ui--css-discipline-blazor) for the full rule.

> [!CAUTION]
> **DOM-first rule for any CSS / layout work.** Before writing or modifying *any* CSS selector, scoped CSS rule, or layout fix in a Blazor component, you **must** first inspect the actually rendered DOM. That means:
>
> 1. Read the component's `.razor` markup to see which elements are authored locally (these receive the scoped `b-xyz` attribute) vs. which are child components (whose rendered roots do **not** receive the parent's scope).
> 2. Read the relevant child components' `.razor` files to know exactly what root element each one emits.
> 3. Only then choose the selector. Prefer container-owned, selector-free layout primitives (`display: flex; gap`, `display: grid`) over sibling selectors and `::deep` chains whenever possible.
>
> Speculative CSS fixes ("this should work") are a defect — they consume tokens, waste user money, and erode trust. If the DOM cannot be inspected from source alone (e.g. dynamic third-party output), say so and ask the user to provide the rendered HTML before proposing a selector. A short pause to verify is always cheaper than a wrong fix.
...
#### UI / CSS Discipline (Blazor)

> [!IMPORTANT]
> These rules apply to all scoped CSS (`*.razor.css`) in the Blazor UI project. They exist to keep the UI **update-safe** against any third-party component library that may be reintroduced in the future. The application currently ships without a third-party UI component library — all UI primitives are project-owned (`Lc*` in `LumaCore.Ui.Components`).

- **No `::deep` selectors that target third-party library internals.** Selectors that reach into the internal layout tree of an external component library (e.g. classes documented as private composition helpers) are **forbidden**. They break silently on every library update.
  - **Allowed:** `::deep` on **own** child classes (e.g., `::deep .lc-conversations-empty-icon`).
  - **Allowed:** `::deep` on a third-party component's documented **public** API class, only if there is no `!important` and no internal layout class involved. Even then, prefer a wrapper component.
  - **Forbidden:** `::deep` on internal layout/composition classes of any third-party library.
- **No `!important` without a justifying comment.** Every `!important` must have a comment on the same line or directly above explaining why standard specificity is insufficient. If the only reason is "to override a third-party library", that is a signal to build a **wrapper component** instead.
- **When the library API does not support a layout requirement, build a wrapper component.** Do not escalate with `::deep` + `!important` against internal classes. After at most one or two attempts that don't work cleanly, **stop** and propose an `Lc*` wrapper component that solves the problem on a class we own.
- **Component naming:** Own UI components live under `src/LumaCore.Ui.Components/Components/` (primitives) and `src/LumaCore.Ui.Web/Components/` (app-specific composites). All are prefixed with `Lc` (e.g., `LcStretchTextArea`, `LcIconButton`, `LcDialog`). They form a stable abstraction layer so a future component library can be plugged in without touching consumers.
- **TODO markers for known hacks:** When a `::deep` + `!important` block on third-party internals cannot be removed immediately, mark the block with `/* TODO(LcComponentName): ... */` describing what will replace it. This makes the technical debt visible and addressable.

### Verification & Review

#### Before Reporting an Issue

Before claiming *any* finding (bug, inconsistency, missing API, wrong behavior), apply these three checks — in order, no shortcuts:

| Check | Question |
|-------|----------|
| **1. Real?** | Read the code again, check related files, look for comments. Is this actually a problem? |
| **2. Intentional?** | Known pattern? Matches BCL behavior? Defensive reasoning? Could there be a reason that's not obvious? |
| **3. Evidence?** | Can you point to specific line numbers? Would "fixing" this break something? |

> [!CAUTION]
> **If ANY doubt remains → ASK, don't claim.** False positives waste time and destroy trust. Better 20 minutes asking than 2 minutes creating false work.

**Stop signals** — if you catch yourself doing any of these, slow down and restart at Check 1:

- Reporting an issue in < 5 minutes of looking
- Using words like "might", "could", "possibly" in the finding
- Can't provide specific line numbers
- Haven't read all related files
- Trusting a comment without verifying the code

#### Code vs. Comments

When a comment contradicts the code: the **code is the source of truth**, the comment is probably outdated. But — *ask first*. Sometimes the comment reveals the bug. Report as: *"Line X comment says Y, but code does Z. Which is correct?"*

#### Proactively Report Inconsistencies

When you spot one of these, surface it (with the smallest consistent fix you can suggest):
...
- Duplicated or contradicting behavior across files
...
> [!IMPORTANT]
> Plan `[Theory]` vs. `[Fact]` **before** writing test code — don't create individual `[Fact]` methods first and consolidate later.

- **Group scenarios by Act+Assert pattern up front.** If multiple scenarios share the same Act call and Assert structure and differ only in inputs/expected outputs, write them as `TheoryData` rows from the start. The goal is reduced boilerplate while preserving line and branch coverage.
- **Per data row:** add a `caseName` / `scenario` string as the first element (readable test output) plus a short comment describing the branch or edge case the row covers.
- **Don't over-generalize.** Keep dedicated `[Fact]` tests when a scenario needs special setup/teardown, distinct assertions, or markedly better readability standalone.
- Prefer `TheoryData<…>` over `IEnumerable<object[]>` for type safety.
- **Test data placement:** when `TheoryData` / `MemberData` lives in the **same file** as its test, place it **before** the test method. If it grows too large, move it to `FooTests.Bar.TestData.cs` (see [Organization Model](#organization-model)).

#### Test Models

Minimize test-only model types — reuse existing ones where possible. New test models go in `FooTests.TestModels.cs` (partial file) and must have XMLDocs explaining their purpose and which scenario they support.
...
1. **Global timeout (10 s)** — configured in `xunit.runner.json`, safety net for unexpected hangs.
2. **`AwaitWithTimeoutAsync` helper** (`src/LumaCore.TestUtilities/Async/AsyncTestHelpers.cs`, default 1 s) — use **only for intentional waits**, where a precise error message beats the generic global timeout.

```csharp
using static LumaCore.TestUtilities.Async.AsyncTestHelpers;
// For sync waits, wrap first: Task waitTask = Task.Run(() => are.Wait());
await AwaitWithTimeoutAsync(waitTask, "WaitAsync timed out");
```

| Operation | Use Helper? | Why |
|-----------|-------------|-----|
| `await are.WaitAsync()` | ✅ Yes | Explicit wait on async primitive |
| `await Assert.ThrowsAsync(() => waitTask)` | ✅ Yes | Waiting for a task to throw |
| Synchronous `.Wait()` on primitive | ✅ Yes | Wrap in `Task.Run` first, then helper |
| `queue.Enqueue()`, `are.Set()`, property access, assertions | ❌ No | Not intentional waits — global timeout covers any hang |

Overloads: `AwaitWithTimeoutAsync(Task, message?, timeout?)` and `AwaitWithTimeoutAsync<T>(Task<T>, message?, timeout?)`.
...
Migration discipline is **phased** and tied to the MVP cut-over:

| Phase | Rule |
|---|---|
| **Pre-MVP (current)** | Create a **new migration** for each schema change using `dotnet ef migrations add <Name>`. **Folding** schema changes into existing migrations (editing the migration `.cs`, its `.Designer.cs`, and the model snapshot in place) is **allowed** because the DB is not yet distributed. |
| **MVP cut-over** | Before the first deploy: delete **all** files under `src/LumaCore.Data/Migrations/` (migrations, designers, and `LumaCoreDbContextModelSnapshot.cs`), then run `dotnet ef migrations add InitialCreate` to produce a single consolidated baseline that matches the current model exactly. Delete local dev/test `.db` files so they regenerate cleanly. |
| **Post-MVP** | **Folding is forbidden.** Every schema change must be a **new** migration. Existing migrations and designer files are append-only history — editing them in place would desynchronize deployed databases from the migration ledger. |

> [!IMPORTANT]
> **TODO(MVP): Consolidate migrations.** When preparing the MVP release, perform the cut-over described above and then update this section to remove the "Pre-MVP" row and the folding allowance. Until then, the folding allowance is a deliberate, time-limited exception — not a long-term pattern.
...
- **Empty sections are omitted.** If a service has no Existence Checks (e.g. Personas), simply skip that `#region`. (Unlike the *comment markers* in [Section Markers](#section-markers), an empty `#region` block is purely decorative — there is nothing inside it to navigate to — so it adds noise without value.)
